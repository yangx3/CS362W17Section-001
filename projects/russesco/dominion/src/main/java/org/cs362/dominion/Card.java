...
//				state.gameBoard.put(getCard(state.cards, CardName.Estate), state.gameBoard.get(getCard(state.cards, CardName.Estate)) - 1);
            ...
            case Cutpurse:
                ...
                for (Player p : state.players) {
                    List<Card> copperCards = filterName(player.hand, CardName.Copper);
            ...
            case GreatHall:
                ...
                //1 draw 1 action (also counts as a Victory Point)
                player.drawCard();
                player.numActions++;
                return;
            ...
            case CouncilRoom:
                ...
                // player draws 3 more and gains a buy.
                player.drawCard();
                player.drawCard();
                player.drawCard();
                player.numBuys++;
...
//				cards.remove(i);
    ...
    public static List<Card> filterName(Iterable<Card> cards, CardName name) {
        List<Card> out = new ArrayList<Card>();
        for (Card c : cards)
            if (c.getCardName() == name) out.add(c);
        return out;
    }
    ...
    public static List<Card> filterObtainable(Iterable<Card> cards, GameState state, int cost) {
        List<Card> out = new ArrayList<Card>();
        for (Card c : cards)
            if (c.getCost() <= cost && c.getCardName() != CardName.Curse && state.gameBoard.get(c) > 0) out.add(c);
        return out;
    }
