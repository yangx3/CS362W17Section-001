...
public class Game {
	 ...
	 */
	public void playCard(Class<? extends Card> cardType) throws InvalidSelectionException{
		if(this.phase==gamePhase.ACTION){ this.playActionPhase(cardType); }
		if(this.phase==gamePhase.BUY){ this.playBuyPhase(cardType); }
	}
	 ...
	 * @throws InvalidSelectionException
	 */
	private void playActionPhase(Class<? extends Card> cardType) throws InvalidSelectionException{	
		try {
			if(!this.handInPlay.canPlay(cardType, this.phase))
				throw new Exceptions.InvalidSelectionException("Can not play selected card.");
			this.handInPlay.playCard(cardType, this.phase);
