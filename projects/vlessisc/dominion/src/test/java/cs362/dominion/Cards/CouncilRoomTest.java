...
public class CouncilRoomTest {
	...
	private Game game;
	private Class<? extends Card> cardType = CouncilRoom.class; 
	...
	public void testPlayCard() {		
		...
		this.game.getCurrentPlayer().deck.hand.addCard(this.game.supply.drawCard(CouncilRoom.class));
		
		try {
			this.game.playCard(CouncilRoom.class);
		} catch (InvalidSelectionException e) {
			fail("Could not play card");
		}
		
		// Assert each player has five cards now
		for(Player player : this.game.getPlayers()){
			if(this.game.getCurrentPlayer()==player){
				assertTrue(player.deck.hand.size()==9);
				continue;
			}
			assertTrue(player.deck.hand.size()==6);
		}
	}
