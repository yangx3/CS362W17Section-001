...
public class testLaboratory
{
	...
	public void test() throws Throwable {
		...
		// Each player plays a Laboratory
		for (Player player : state.players)
		{
			...
			c.play(player, state);
			// Check that hand goes up by two and deck goes down same
			assertEquals( player.hand.size(), startstate.players.get(i).hand.size()+2 );
			assertEquals( player.deck.size(), startstate.players.get(i).deck.size()-2 );
			// Verify player actions went up one
			assertEquals( player.numActions, startstate.players.get(i).numActions+1 );
			i = 1;
		}
	}
}
